def oekonomie_vorbereiten_gw(strompreis, kW, strompreissteigerung):
    # Imports
    import numpy as np
    ...
    # Invest
    if kW >= 30:
        eco["invest"] = np.round(1.923E+03 * kW ** (-0.16) * kW * 1.19, 2) + 3000
    else: 
        eco["invest"] = np.round(1.923E+03 * kW ** (-0.16) * kW * 1.19, 2)
    # EEG Umlage
...
def oekonomie_vorbereiten_gw_ds(strompreis, kW, strompreissteigerung):
    # Imports
    import numpy as np
    ...
    eco["betrieb"] = c_fix + c_zaehler + c_pro_leist * kW + c_messstelle

    invest_zaehler = 300*i_teilnehmer
    invest_pv = 1750*kW**(-0.1296)*kW*1.19
    if kW >= 30:
...
def oekonomie_vorbereiten_gw_ve(kW):
    import numpy as np
...
def oekonomie_berechnen_gw_ds(leistung_pv, leistung_last, eco, kW, kalkulatorischer_zins, betreiber, Jahresstromverbrauch):
    ...
    summe_pvs = np.sum(leistung_pv) / (4*1000)
...
def oekonomie_berechnen_gw_ve(leistung_pv, eco, kW, kalkulatorischer_zins):
    ...
    if kW == 0:
        ...
        rendite = np.round((gewinnkurve[-1]) / (-1 * gewinnkurve[0]), 1)
        rendite *= 100
    return nettobarwert, rendite, gewinnkurve
